	...
	if($_POST['bidamount']){
		...
		if(!preg_match('/^[0-9]*\.?[0-9]+$/',$amount) || number_format($amount,2)=='0.0' || strlen($amount)==0){					
			...
			if(empty($showresult)){		
				...
				$showresult = $dbMaster->getRowAsAssoc($connect,$sql);
				if(!empty($showresult)){
					$error .='El usuario esta bloqueado para ofertar.<br>';
				}
				if($bloqued == 1){
					$error .='Las ofertas ya se encuentran bloqueadas.<br>';
				}
			}
		}
		...
		if($error==""){
			...
			if($_POST['enviacorreo'] == "1"){
				...
				le informamos que su oferta por el lote '.$numberlot.', ha sido superada,<br> puede ingresar a: subastas.maxilana.com y checar el status de ofertas,<br>
